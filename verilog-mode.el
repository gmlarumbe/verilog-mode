  ...
  (and (if discard-re
       ...
       (if (save-excursion
           ...
           (save-excursion
             ...
             (looking-at (verilog-get-declaration-re)))
         t)
       ;; Ensure it's not any kind of logical comparison inside some 'with' block
       ;; (let ((pos (point)))
       ;;   (save-excursion
       ;;     (unless (and (verilog-re-search-forward verilog-assignment-operation-re (point-at-eol) 'move)
       ;;                  (verilog-re-search-backward "{" pos 'move))
       ;;       t)))
...
If QUIET is non-nil, do not print messages showing the progress of line-up."
  (interactive)
  (let* ((basic-complete-pretty-expr-words ; Same as `verilog-basic-complete-re' but excluding default, parameter and localparam keywords
          '("always" "assign" "always_latch" "always_ff" "always_comb" "analog" "connectmodule" "constraint"
            "import" "initial" "final" "module" "macromodule" "repeat" "randcase" "while"
            "if" "for" "forever" "foreach" "else" "do" "assert" "generate"))
         (basic-complete-pretty-expr-re (verilog-regexp-words basic-complete-pretty-expr-words))
         (complete-pretty-expr-re (concat verilog-extended-complete-re "\\|\\(" basic-complete-pretty-expr-re "\\)"))
         (basic-complete-pretty-expr-no-assign-words (remove "assign" basic-complete-pretty-expr-words))
         (basic-complete-pretty-expr-no-assign-re (verilog-regexp-words basic-complete-pretty-expr-no-assign-words))
         (complete-pretty-expr-no-assign-re (concat verilog-extended-complete-re "\\|\\(" basic-complete-pretty-expr-no-assign-re "\\)"))
         (discard-re (if verilog-align-assign-expr
                         complete-pretty-expr-no-assign-re
                       complete-pretty-expr-re))
        ...
        (when (and (verilog--pretty-expr-assignment-found discard-re-line)
          ...
          (let* ((start (if (verilog-in-parenthesis-p)
                            ...
                            (progn
                              ...
                              (verilog-re-search-forward (concat "\\(.*?\\)" verilog-assignment-operator-re) nil 'move)
                              (goto-char (match-beginning 0))
              ...
              (save-excursion
                (if (looking-at verilog-complete-re)
                    (progn (goto-char (marker-position startpos))
                           (verilog-just-one-space (concat "\\(.*?\\)" verilog-assignment-operator-re)))
                ...
                (cond
                 ((or (looking-at verilog-assignment-operation-re)
                      (and (verilog-in-parenthesis-p)
                           (looking-at (concat "\\(.*?\\)" verilog-assignment-operator-re))))
